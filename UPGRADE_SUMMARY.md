...
**File**: [recommender.py](recommender.py)
...
### Why It Matters
- A ₹300 shirt won't recommend a ₹5,000 blazer just because words match
...
**File**: [favorites_manager.py](favorites_manager.py)

### What Changed
- **Before**: `st.session_state` (lost on refresh)
- **After**: **Local SQLite database** (`stylesync_favorites.db`)

### User Experience Fix
| Before | After |
|--------|-------|
| Add favorite → refresh → gone ❌ | Add favorite → persist forever ✅ |
| All data in RAM | All data in .db file |
| No feedback tracking | Tracks likes/dislikes |
...
**File**: [recommender.py](recommender.py) + [app.py](app.py)
...
### Example Output
```
✨ 🏷️ similar price | ⭐ highly rated | 🎉 30% off
```
...
def get_recommendation_reason(self, query_idx, candidate_idx) -> str:
    ...
    if abs(candidate['price'] - query['price']) < 500:
        reasons.append("🏷️ similar price")
    if candidate['rating'] >= 4.5:
        reasons.append("⭐ highly rated")
    if candidate['discount'] > 30:
        reasons.append(f"🎉 {candidate['discount']}% off")
    return " | ".join(reasons)
```
...
**File**: [search_history.py](search_history.py) + [app.py](app.py) sidebar

### What Changed
App now **learns** from user behavior and shows a style profile in the sidebar.

### Profile Inferred From Search History
- **Your Average Budget**: ₹800–₹1,200  
- **Preferred Rating**: 4.2⭐  
...
**File**: [favorites_manager.py](favorites_manager.py) + [app.py](app.py)
...
2. Clicks 👎 → product never shows again
3. Clicks 👍 → seller is boosted in future rankings
...
The app will:
1. Create `stylesync_favorites.db` on first run
...
## 📁 **New Files**
- **[search_history.py](search_history.py)** — Manages search history and user profiling
- **[stylesync_favorites.db](stylesync_favorites.db)** — Created on first run (SQLite database)

## ✏️ **Modified Files**
- **[recommender.py](recommender.py)** — Added `_hybrid_score()` and `get_recommendation_reason()`
- **[favorites_manager.py](favorites_manager.py)** — Rewritten with SQLite backend
- **[app.py](app.py)** — Integrated all new features into UI
- **[requirements.txt](requirements.txt)** — Updated with v2.1 notes
...
- [ ] Add to favorites → refresh → still there ✅
- [ ] See recommendation explanation under each product
- [ ] Click 👍/👎 and check search history
