...
def load_data():
    ...
    try:
        loader = DataLoader(csv_path="myntra.csv", sample_size=10000)
        return loader.get_dataframe(), loader.get_stats()
    except FileNotFoundError:
        st.error(
            "Dataset file 'myntra.csv' not found. Please ensure it's in the project directory.")
        st.stop()
...
if st.session_state.get('show_favorites', False):
    ...
    search_query, min_price, max_price, min_rating, min_discount, occasion, gender = get_main_search_filters()
    ...
    if st.session_state.get('search_executed', False):
        ...
        if not search_query:
            ...
            with st.spinner("Finding similar products..."):
                results = recommender.recommend(search_query, top_n=10)
